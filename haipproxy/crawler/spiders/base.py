...
class BaseSpider:
    ...
    # 爬虫的一些配置信息
    custom_settings = {
        'CONCURRENT_REQUESTS_PER_DOMAIN': 5,  # 对单个网站进行并发请求的最大值。

        'DOWNLOAD_DELAY': 3,  # 下载器在下载同一个网站下一个页面前需要等待的时间。该选项可以用来限制爬取速度， 减轻服务器压力
        'EXTENSIONS': {
    ...
    def parse_json(self, response, detail_rule, ip_key='ip', port_key='port'):
        ...
        # json, 解析
        # response的body是字节，现在给转换成utf8 => str
